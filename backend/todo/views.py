...
SYSTEM_PROMPT = "Ты — помощник по генерации задач. Сформируй задачи по тексту пользователя."
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            try:
                ...
                logger.info('Task created: %s', instance)
                return Response(TaskSerializer(instance).data, status=status.HTTP_201_CREATED)
...
class MistralView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            system_prompt = (
                "Ты — помощник по генерации задач. Верни JSON вида:\n"
                "{\n"
                '  "tasks": [\n'
                "    {\n"
                '      "id": "1",\n'
